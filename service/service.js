...
export const buscarPorAno = (ano) => {
  ...
  return historicoInflacao.filter(
    (historico) => historico.ano === historicoAno
  );
};

//Função que calcula o reajuste do IPCA
export const calculoIPCA = (
  valorInicial,
  anoInicial,
  mesInicial,
  anoFinal,
  ...
  if (
    [valorInicial, anoInicial, mesInicial, anoFinal, mesFinal].some(
  ...
  if (isNaN(vi) || isNaN(ai) || isNaN(mi) || isNaN(af) || isNaN(mf)) {
    return {
      erro: "Parâmetros inválidos. O valor deve ser um número (ex: 100.00), o ano no formato 'aaaa' e o mês no formato 'mm'.",
    };
  }
  ...
  while (ano < af || (ano === af && mes <= mf)) {
    const ipcaEntrada = historicoInflacao.find(
      (e) => e.ano === ano && e.mes === mes
    );
    
    if (!ipcaEntrada) {
      return { erro: `Dados do IPCA não encotrados para ${mes}/${ano}` };
    }
    indiceAcumulado *= 1 + ipcaEntrada.ipca / 100;
